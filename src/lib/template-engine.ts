...
export function generateExperienceItems(
  ...
  const templateGenerators: Record<string, (exp: typeof cvData.experience[0]) => string> = {
    ...
    'experience-glass-modern': (exp) => `<article class="exp-glass-card"><div class="exp-glass-header"><div><h3 class="exp-glass-position">${escapeHtml(exp.position)}</h3><div class="exp-glass-company">${escapeHtml(exp.company)}</div></div><time class="exp-glass-duration" datetime="${escapeHtml(exp.duration)}">${escapeHtml(exp.duration)}</time></div><p class="exp-glass-description">${escapeHtml(exp.description)}</p></article>`,
    'experience-animated-gradient': (exp) => `<article class="exp-gradient-card"><div class="exp-gradient-header"><div><h3 class="exp-gradient-position">${escapeHtml(exp.position)}</h3><div class="exp-gradient-company">${escapeHtml(exp.company)}</div></div><time class="exp-gradient-duration" datetime="${escapeHtml(exp.duration)}">${escapeHtml(exp.duration)}</time></div><p class="exp-gradient-description">${escapeHtml(exp.description)}</p></article>`
  };
